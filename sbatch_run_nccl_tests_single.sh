...
Options:
  ...
  -l, --log-dir <DIR>        Directory for logs. Default: benchmarks/<CLUSTER>/nccl-benchmark-results/runs/<RUN_ID>/single-node/without-debug/logs
...
if [[ $DEBUG -eq 1 ]]; then
  LOG_DIR=${LOG_DIR:-"$RESULTS_DIR/single-node/with-debug/logs"}
else
  LOG_DIR=${LOG_DIR:-"$RESULTS_DIR/single-node/without-debug/logs"}
fi
